...
function renderMovieGrid(container, movies) {
  const gridHTML = movies
    .map(
      (movie) => `
        <div class="movie-item bg-gray-800 p-4 rounded-lg">
            <img src="${movie.poster}" alt="${movie.title}" class="w-full h-40 object-cover rounded mb-2">
...
function loadEpisodes() {
  if (!currentMovie.multipleDownloads) return
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <span class="text-white ml-2">${currentMovie.director}</span>
                    </div>
                    <div>
...
function loadDownloadsTab() {
                  ...
                  .map(
                    ...
                    <div class="bg-gray-800 p-4 rounded-lg">
                        <div class="flex items-center justify-between">
                            <div>
                                <h4 class="text-white font-medium">${episode.label}</h4>
                                <p class="text-gray-400 text-sm">Multiple qualities available</p>
                            </div>
                            <div class="flex space-x-2">
                                <button onclick="downloadEpisode(${index}, '4K')" class="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded text-sm">
                                    4K
                                </button>
                                <button onclick="downloadEpisode(${index}, 'HD')" class="bg-gray-600 hover:bg-gray-700 text-white px-3 py-1 rounded text-sm">
                                    HD
                                </button>
                                <button onclick="downloadEpisode(${index}, 'SD')" class="bg-gray-600 hover:bg-gray-700 text-white px-3 py-1 rounded text-sm">
                                    SD
                                </button>
                            </div>
                        </div>
            ...
            <div class="space-y-4">
                <div class="bg-gray-800 p-4 rounded-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <button onclick="downloadMovie('4K')" class="bg-red-600 hover:bg-red-700 text-white p-3 rounded text-center">
                            <div class="font-bold">4K Ultra HD</div>
                            <div class="text-sm opacity-75">~8-15 GB</div>
                        </button>
                        <button onclick="downloadMovie('HD')" class="bg-gray-600 hover:bg-gray-700 text-white p-3 rounded text-center">
                            <div class="font-bold">Full HD</div>
                            <div class="text-sm opacity-75">~2-4 GB</div>
                        </button>
                        <button onclick="downloadMovie('SD')" class="bg-gray-600 hover:bg-gray-700 text-white p-3 rounded text-center">
                            <div class="font-bold">Standard</div>
                            <div class="text-sm opacity-75">~500MB-1GB</div>
                        </button>
                    </div>
                </div>
            </div>
  ...
  if (desktopQuickDownload) {
    desktopQuickDownload.innerHTML = `
            <div class="space-y-2">
                <button onclick="downloadMovie('4K')" class="w-full bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded text-sm">
                    Download 4K
                </button>
                <button onclick="downloadMovie('HD')" class="w-full bg-gray-600 hover:bg-gray-700 text-white py-2 px-4 rounded text-sm">
                    Download HD
                </button>
                <button onclick="downloadMovie('SD')" class="w-full bg-gray-600 hover:bg-gray-700 text-white py-2 px-4 rounded text-sm">
                    Download SD
                </button>
            </div>
        `
...
function loadCastTab() {
  ...
  if (!desktopCast || !currentMovie.cast) return
...
function downloadMovie(quality = "HD") {
  showToast(`Starting download: ${currentMovie.title} (${quality})`, "success")
  // In a real implementation, this would trigger the actual download
}

function downloadEpisode(episodeIndex, quality = "HD") {
  const episode = currentMovie.multipleDownloads[episodeIndex]
  showToast(`Starting download: ${episode.label} (${quality})`, "success")
  // In a real implementation, this would trigger the actual download
}
